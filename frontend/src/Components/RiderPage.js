...
function RiderPage() {
  ...
  useEffect(() => {
    ...
    if (socketInstance.current === null) {
      const socket = io("https://full-stack-rideshare.vercel.app/api/rider", {
  ...
  async function changeInstructions(id, name) {
    ...
    let tempLink = `https://full-stack-rideshare.vercel.app/api/rideinfo/rider/${id}/${tempInstruct}/${name}`;
    try {
  ...
  async function request_ride(event) {
    ...
    event.preventDefault();
    let tempLink = `https://full-stack-rideshare.vercel.app/api/singlerider/${data.id}/${data.name}/0,0/${tempDest}/${socketInstance.current.id}`;
  ...
  async function finishRide(rider_id, rating_of_driver, review_of_driver) {
    let tempLink = `https://full-stack-rideshare.vercel.app/api/singlerider/post/${rider_id}/0/${rating_of_driver}/${review_of_driver}`;
    try {
      const response = await fetch(tempLink, {
        method: "POST",
        mode: "cors",
